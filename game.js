...
async function myFunc(str) {
    try {  
      const response = await fetch("https://api.twitch.tv/helix/clips?game_id=" + str + "&started_at=" + start_year + "-" + start_month + "-"+ start_day + "T00:00:00Z&ended_at=" + end_year + "-" + end_month + "-" + end_day + "T00:00:00Z", {
        method: 'GET',
        headers: {
...
readline.question('Enter a game ID and how many days back to search separated by a colon:\n', name => {
    dt = new Date()
    ...
    myFunc(name.substring(0,name.indexOf(":")))
    readline.close();
});
